"""
Akul| TM Simulator: ww
Nov 22,2020

Added the the language of balanced parentheses.
...
if __name__ == "__main__":
    ...
    m = TuringMachine('10101011010101', [9])

    #m.addTransition(0,'_',9,'_','R')

    m.addTransition(0,'0',1,'X','R')
    m.addTransition(0,'1',1,'Y','R')

    m.addTransition(1,'1',1,'1','R')
    m.addTransition(1,'0',1,'0','R')

    m.addTransition(1,'Y',2,'Y','L')
    m.addTransition(1,'X',2,'X','L')
    m.addTransition(1,'_',2,'_','L')

    m.addTransition(2,'0',3,'X','L')
    m.addTransition(2,'1',3,'Y','L')

    m.addTransition(3,'1',3,'1','L')
    m.addTransition(3,'0',3,'0','L')

    m.addTransition(3,'X',0,'X','R')
    m.addTransition(3,'Y',0,'Y','R')

    m.addTransition(0,'X',4,'X','L')
    m.addTransition(0,'Y',4,'Y','L')

    m.addTransition(4,'X',4,'0','L')
    m.addTransition(4,'Y',4,'1','L')

    m.addTransition(4,'_',5,'_','R')

    m.addTransition(5,'0',6,'X','R')

    m.addTransition(6,'0',6,'0','R')
    m.addTransition(6,'1',6,'1','R')
    m.addTransition(6,'_',6,'_','R')

    m.addTransition(6,'X',8,'_','L')

    m.addTransition(5,'1',7,'Y','R')
    
    m.addTransition(7,'1',7,'1','R')
    m.addTransition(7,'0',7,'0','R')
    m.addTransition(7,'_',7,'_','R')

    m.addTransition(7,'Y',8,'_','L')

    m.addTransition(8,'0',8,'0','L')
    m.addTransition(8,'1',8,'1','L')
    m.addTransition(8,'_',8,'_','L')

    m.addTransition(8,'X',5,'X','R')
    m.addTransition(8,'Y',5,'Y','R')

    m.addTransition(5,'_',9,'_','L')
